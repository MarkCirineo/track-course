...
type HoleScoreForPlay = {
    holeId: string;
    score: number;
    hole: { holeIndex: number };
};
...
export function PlayScorecardModal({
    ...
    const scoreByHoleId = new Map(
        play.holeScores.map(hs => [hs.holeId, hs.score])
    );
    ...
    const getDistance = (hole: HoleForScorecard) =>
        holeTee(hole)?.distance ?? null;
    const getStrokeIndex = (hole: HoleForScorecard) =>
        holeTee(hole)?.strokeIndex ?? null;
    const getScore = (hole: HoleForScorecard) =>
        scoreByHoleId.get(hole.id) ?? null;
    ...
    const ScoreCell = ({
        ...
        if (style === "double-square")
            return (
                <span
                    className={cn(
                        base,
                        "size-8 rounded-sm border p-0.5",
                        border
                    )}
                >
                    <span
        ...
        if (style === "single-square")
            return (
                <span
                    className={cn(
                        base,
                        "size-7 rounded-sm border px-1 text-sm",
                        border
                    )}
                >
                    {content}
        ...
        if (style === "single-circle")
            return (
                <span
                    className={cn(
                        base,
                        "size-7 rounded-full border px-1 text-sm",
                        border
                    )}
                >
                    {content}
        ...
        if (style === "double-circle")
            return (
                <span
                    className={cn(
                        base,
                        "size-8 rounded-full border p-0.5",
                        border
                    )}
                >
                    <span
        ...
        return (
            <span className={cn(base, "min-w-[1.5rem] text-sm")}>
                {content}
            </span>
        );
    };
                                                ...
                                                <div className="space-y-2 text-xs">
                                                    <div className="flex items-center gap-2">
                                                        <span className="inline-flex size-5 items-center justify-center rounded-full border-2 border-background p-0.5">
                                                            <span className="flex size-3 items-center justify-center rounded-full border border-background text-[10px]">
                                                                2
                                                            </span>
                                                        </span>
                                                        <span>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <span className="inline-flex size-5 items-center justify-center rounded-full border-2 border-background text-[10px]">
                                                            3
                                                        </span>
                                                        <span>Birdie</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <span className="inline-flex min-w-[1rem] items-center justify-center text-[10px]">
                                                            4
                                                        </span>
                                                        <span>Par</span>
                                                    </div>
